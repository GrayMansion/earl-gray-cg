...
void cg::renderer::dx12_renderer::create_render_target_views()
{
	...
	for (UINT i = 0; i < frame_number; i++)
	{
		...
		device->CreateRenderTargetView(render_targets[i].Get(),
			nullptr, rtv_heap.get_cpu_descriptor_handle(i));
...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC* sampler_descriptors, UINT num_sampler_descriptors)
{
	...
	if (FAILED(device->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, &data,
		sizeof(data))))
	{
	...
	desc.Init_1_1(_countof(root_parameters), root_parameters,
		num_sampler_descriptors, sampler_descriptors, flags);
...
void cg::renderer::dx12_renderer::create_pso()
{
	...
	D3D12_INPUT_ELEMENT_DESC input_descs[] = {
		...
		{"COLOR", 1, DXGI_FORMAT_R32G32B32_FLOAT, 0,
			44,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},

		{"COLOR", 2, DXGI_FORMAT_R32G32B32_FLOAT, 0,
			56,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0}
	};
